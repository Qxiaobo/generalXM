        ...
        <el-aside style="width: 400px">
          <el-menu :default-active="$route.path" background-color="#fff" active-text-color="#1890ff"
            ...
            <el-submenu :index="'/' + item.path" v-for="(item, index) in routes" :key="index">
              ...
              <el-menu-item :index="'/' + j.path" :title="j.title" v-for="(j, i) in item.children" :key="i"
                class="everyRouter" @click="toEveryRouter(j)">{{ j.title }}
              </el-menu-item>
            </el-submenu>
          </el-menu>
        </el-aside>
...
export default {
  ...
  mounted() {
    // console.log(this.$store.state.navList, '当前面包屑数组');
    // console.log(this.$store.state.currentNav, '当前路由');
  },
  computed: {
    ...mapState({
