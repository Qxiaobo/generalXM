/* eslint-disable */
window.tinymce.PluginManager.add('axupimgs', function(editor, url) {
  var pluginName = 'Ax多图片上传';
  window.axupimgs = {};
  var baseURL = window.tinymce.baseURL;
  var iframe1 = baseURL + '/packages/text-editor/src/plugins/axupimgs/upfiles.html';
  window.axupimgs.images_upload_handler = editor.getParam('images_upload_handlerAx', undefined, 'function');
  window.axupimgs.images_upload_base_path = editor.getParam('images_upload_base_path', '', 'string');
  window.axupimgs.axupimgs_filetype = editor.getParam('axupimgs_filetype', '.png,.gif,.jpg,.jpeg', 'string');
  window.axupimgs.res = [];
